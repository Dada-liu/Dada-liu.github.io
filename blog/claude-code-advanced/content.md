...
我使用的是 deepseek 的 api，但是因为 deepseek 没有图片识别的能力，我只能使用其他模型；

本来想用 claude opus 来做，但是是搞了半天没搞定 api key，放弃；这里推荐一下中转站 OpenRouter（名气大、注册方便、可以用支付宝/微信充值，充10💲还能享受免费模型）；

于是我使用了国内的模型，下面将会展示多个模型的效果；


## 二、使用 Skills 来实现自动化
Claude Code 提供了 Skills 实现工作流的复用，于是我写了一个 简历图片→简历编辑器 的 Skill：
